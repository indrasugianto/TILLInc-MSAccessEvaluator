-- Fix applied: STEP 11 changed FROM tblStaff to FROM #tempstaff
-- so Dedham Manager adjusted departments match tblExpirations.Location
-- This fixes blank skill dates (CPR, FirstAid, etc.) for 303+ Dedham Manager staff

ALTER PROCEDURE [dbo].[spApp_RunExpirationReport]
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
    ...
    BEGIN TRY
        ...
        -- FIX #2: Insert additional ISO locations only if not already exists
        ...
        WHERE loc.CityTown <> 'Dedham'
            ...
            AND NOT EXISTS (
                SELECT 1
                FROM #temptbl t
                WHERE t.Location = loc.CityTown + ' - ' + loc.LocationName
            )
        ORDER BY loc.CityTown + ' - ' + loc.LocationName;
        ...
        -- ========================================
        -- FIX #4: Add DISTINCT to prevent duplicate clients with multiple locations of same service type
        -- CLO Clients
        INSERT INTO tblExpirations (
            Location, RecordType, LastName, FirstName, Supervisor,
        ...
        -- ========================================
        -- FIX #1: Changed INNER JOIN to EXISTS to prevent duplicates from multiple skills
        INSERT INTO tblExpirations (
        ...
        FROM #tempstaff ts
        WHERE EXISTS (SELECT 1 FROM #tempstaffskills tss WHERE ts.EMPLOYID = tss.EMPID_I)
            AND ts.DEPRTMNT IS NOT NULL
        ...
        -- FIX #5: Use #tempstaff instead of tblStaff so Dedham Manager
        -- adjusted departments match tblExpirations.Location
        -- (Previously used tblStaff which has original DEPRTMNT values,
        --  causing JOIN mismatch for 303 Dedham Manager staff)
        SELECT
        ...
        -- ========================================
        -- FIX #3: Added DISTINCT to prevent duplicate supervisors from multiple locations
        INSERT INTO tblExpirations (
